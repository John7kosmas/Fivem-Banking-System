...
function CreateMenu(info)
    ...
    local item = {
        ...
        ---@type table
        Color = {
            R = U:Ensure(info.R or info.r, 118),
            G = U:Ensure(info.G or info.g, 0),
            B = U:Ensure(info.B or info.b, 194)
        },
        ...
        ToTable = function(t)
            local tempTable = {
                ...
                color = {
                    r = U:Ensure(t.Color.R, 118),
                    g = U:Ensure(t.Color.G, 0),
                    b = U:Ensure(t.Color.B, 194)
                },
                items = {}
            }
            ...
            if (tempTable.color.r <= 118) then tempTable.color.r = 118 end
            if (tempTable.color.r >= 118) then tempTable.color.r = 118 end
            if (tempTable.color.g <= 0) then tempTable.color.g = 0 end
            if (tempTable.color.g >= 0) then tempTable.color.g = 0 end
            if (tempTable.color.b <= 194) then tempTable.color.b = 194 end
            if (tempTable.color.b >= 194) then tempTable.color.b = 194 end
